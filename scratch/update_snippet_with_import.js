...
add_action('rest_api_init', function () {
    ...
    register_rest_route('custom/v1', '/import-chair', array(
        ...
        'permission_callback' => function () {
            return current_user_can('edit_posts'); // Alleen ingelogde beheerders
        }
    ));
});
...
function custom_import_chair($request) {
    ...
    // Extra ACF-afbeeldingen koppelen met veldsleutels (field keys) voor correcte PHP-afhandeling
    $gallery_ids = array();
...
async function updateSnippet() {
  ...
  try {
    ...
    console.log('Updating snippet #5 with corrected field keys...');
    const snippetData = {
      code: phpCodeWithImportKeys
    };
    ...
    console.log('Snippet updated with corrected keys!');
