...
function get_custom_chairs_json() {
    ...
    foreach ($products as $post) {
        ...
        // Extra afbeeldingen ophalen via ACF, met fallback naar get_post_meta met ID-naar-URL conversie
...
function custom_import_chair($request) {
    ...
    // Extra ACF-afbeeldingen koppelen met media-ID's
    $gallery_ids = array();
...
async function updateSnippet() {
  ...
  try {
    ...
    console.log('Updating snippet #5 with correct ID mapping...');
    const snippetData = {
      code: phpCodeCorrectIds
    };
